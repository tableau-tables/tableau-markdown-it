...
function tableau_table_plugin(md, _options) {
    md.block.ruler.at('table', table, { alt: ['paragraph', 'reference'] });
}
exports.default = tableau_table_plugin;
////////////////////////////////////////////////////////////////////////
...
function render_row(state, row) {
    ...
    row.cells.forEach(cell => render_cell(state, cell, token.level + 1));
...
//   ">": "a-r",
// }
function render_cell(state, cell, level) {
    const tag_name = cell.format.heading ? "th" : "td";
    state.push(`${tag_name}_open`, tag_name, 1);
    const token = state.push("inline", "", 0);
